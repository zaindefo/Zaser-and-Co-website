  ...
  return (
    <main>
      ...
      <section className="audit-dimensions section section--navy" data-nav-theme="dark">
        ...
        <div className="audit-dimensions__layout"><div className="audit-dimensions__list">{audit.dimensions.map((item, index) => <article key={item.title}><span>{String(index + 1).padStart(2, '0')}</span><h3>{item.title}</h3><p>{item.description}</p></article>)}</div><ArtifactVisual type={audit.inquiryType === 'ai-audit' ? 'scorecard' : 'diagnostic'} label={`${audit.title} diagnostic dimensions`} dark /></div>
      </section>
